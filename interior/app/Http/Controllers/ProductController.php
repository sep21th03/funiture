...
class ProductController extends Controller
{
    ...
    private function prepareProductsData($products)
    {
        ...
        // });
        return $products->map(function ($product) {
            $firstProductHex = $product->productHex->first(); // Lấy product_hex đầu tiên
            $firstSize = $firstProductHex?->sizes->first(); // Lấy size đầu tiên
            $firstGallery = $firstProductHex?->galleries->first(); // Lấy ảnh đầu tiên
            ...
            'data' => [
                ...
                'product_hex' => $product->productHex->map(function ($hex) {
                    return [
                        ...
                        'image' => optional($hex->galleries->first())->image_path ?? null,
                    ];
                }),
                'reviews' => []
            ]
